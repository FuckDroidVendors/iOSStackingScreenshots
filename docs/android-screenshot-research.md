...
- Build a tiny Android spike to test secure overlay behavior empirically.
- If exact behavior is mandatory, pivot quickly to platform-level work instead of spending too much time on public-SDK-only designs.
- Audit AOSP screenshot and SystemUI code to determine where to tag and exclude the preview layer with minimal patch size.
